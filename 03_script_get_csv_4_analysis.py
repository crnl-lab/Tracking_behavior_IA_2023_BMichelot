...
"""this script take as input the output of 01_script_loader or 02_script_interpolate. this script aims to get the data for machine learning analysis.
Column "Condition" : see event code in functions_get_csv_4_analysis"""
"""Module : Normalization (below) and Transformation (functions_get_csv_4_analysis)"""
...
on_subs = cfg.some_subs  # could be cfg.all_subjects or cfg.some_subs
...
for sub in on_subs:
    ...
    for filename in os.listdir(sub_CSVs_path):
    #list_of_files = sorted(filter(os.path.isfile, glob.glob(sub_CSVs_path + '*' + data_name_ending)))
    #for path in list_of_files:

        if filename.endswith(data_name_ending):

            #current working data and event (one csv for one subject)
            data = pd.read_csv(sub_CSVs_path + filename)

            # Data relative body point : NORMALIZATION #
            ...
            part= [cfg.pose, cfg.face, cfg.gaze_angle, cfg.head_orientation, cfg.head_position, cfg.AUr]
...
df = fct.adapt_csv_format(label='EmotionOnly', normalize_scales=True, write=True)
